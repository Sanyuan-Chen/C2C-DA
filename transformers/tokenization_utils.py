...
class PreTrainedTokenizer(object):
    ...
    def decode(self, token_ids, skip_special_tokens=False, clean_up_tokenization_spaces=True):
        ...
        for token in filtered_tokens:
            ...
            if token in self.added_tokens_encoder:
                ...
                is_last_added_token = True
            else:
                current_sub_text.append(("Ġ" if is_last_added_token else "") + token)
                is_last_added_token = False
    ...
    def clean_up_tokenization(out_string):
        ...
        out_string = out_string.replace(' .', '.').replace(' ?', '?').replace(' !', '!').replace(' ,', ','
                        ).replace(" ' ", "'").replace(" do not", " don't") \
            .replace(' <|endoftext|>', '<|endoftext|>').replace('<|endoftext|> ', '<|endoftext|>')\
            .replace(' <|endoftext|> ', '<|endoftext|>').replace('<|endoftext|>', ' <|endoftext|> ')
            # .replace(" 'm", "'m").replace(" 's", "'s").replace(" 're", "'re").replace(" 've", "'ve").replace(" n't", "n't")\
        return out_string
